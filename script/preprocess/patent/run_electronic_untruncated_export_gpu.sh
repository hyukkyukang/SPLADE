...
RUN_DIR="${RUN_DIR:-outputs/patent/electronic_untruncated_val_mrr_09777_gpu_run}"
...
if [[ "${SHARD_COUNT}" -ne "${#GPU_IDS[@]}" ]]; then
  echo "[ERROR] SHARD_COUNT must match the number of GPU ids for this launcher."
  exit 1
fi
...
python - <<'PY' "${SHARD_DIR}" "${FINAL_OUTPUT}" "${SHARD_COUNT}" "${BASE_MODEL}" "${CHECKPOINT_PATH}" "${DOCUMENT_BATCH_SIZE}" "${ENCODE_BATCH_SIZE}" "${DATALOADER_NUM_WORKERS}" "${GPU_IDS_CSV}" "${DOCUMENT_ENCODING_MODE}" "${TERM_OUTPUT_MODE}"
import json
import sys
...
document_encoding_mode = sys.argv[10]
term_output_mode = sys.argv[11]
