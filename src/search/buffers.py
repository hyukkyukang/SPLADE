...
def resolve_query_sparsify_config(
    ...
    """Return raw tokenizer token exclusions and sparsify thresholds from config."""
    exclude_value: Any = cfg.testing.query_exclude_token_ids
    if exclude_value:
        source_exclude_token_ids = [int(token_id) for token_id in exclude_value]
    else:
    ...
    return source_exclude_token_ids, min_weight_value, top_k
