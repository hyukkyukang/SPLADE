"""Per-task instruction prompts for MTEB evaluation, mirroring the official
Yibin-Lei/LENS `eval/instruction.py` verbatim.

Used by our LENS MTEB runner so that each task's query receives the same
instruction string that was used to produce the paper's numbers.
"""
...
# Transcribed verbatim from the official repo. Keys that map to a tuple mean
# ``(query_instruction, corpus_instruction)``; LENS uses only the query side
# for all MTEB tasks.
_TASKNAME2INSTRUCTIONS: dict[str, str | tuple[str, str]] = {
    # BitextMining
    ...
    # Retrieval — (query_instruction, corpus_instruction)
...
def task_to_instruction(task_name: str, is_query: bool = True) -> str:
    """Return the instruction prompt for an MTEB task.

    Mirrors ``Yibin-Lei/LENS/eval/instruction.py::task_to_instruction``:
    - Tuple entries: ``[0]`` is query instruction, ``[1]`` is corpus instruction.
    - String entries: the same instruction is returned regardless of ``is_query``.
    - Unknown tasks fall back to ``DEFAULT_PROMPTS[meta.type]`` when the
      ``mteb`` library can resolve the task metadata. Otherwise we return
      an empty string.
    The returned instruction is always terminated with a period.
    """
    if task_name in _TASKNAME2INSTRUCTIONS:
        entry: str | tuple[str, str] = _TASKNAME2INSTRUCTIONS[task_name]
        if isinstance(entry, tuple):
            if is_query:
                return _ensure_period(entry[0])
            return entry[1]
        return _ensure_period(entry)
    ...
    try:
        ...
        return ""
    task_type: str = getattr(meta, "type", "")
    default_prompt: str = _DEFAULT_PROMPTS.get(task_type, "")
    return _ensure_period(default_prompt)


__all__ = ["task_to_instruction"]
